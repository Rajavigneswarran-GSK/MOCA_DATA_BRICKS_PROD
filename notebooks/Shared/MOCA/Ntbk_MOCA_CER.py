...
F_DOCUMENTS= spark.sql("""SELECT DOCUMENT_NUMBER,DOCUMENT_TYPE_ID,CAMPAIGN_ID,DOCUMENT_VERSION,CONTENT_PURPOSE,ROW_NUMBER() OVER(PARTITION BY DOCUMENT_NUMBER ORDER BY DOCUMENT_VERSION DESC) AS REC_RK   FROM MOCA.F_DOCUMENTS """)
F_DOCUMENTS.write.mode("overwrite").option("overwriteSchema",'true').saveAsTable("MOCA.F_DOCUMENTS_STG")
...
,nvl(DA1.SOURCE_ACCOUNT_AID,FM.SOURCE_GIGYA_AID) AS CUSTOMER_ID
...
DISTINCT
EA.country_id
,nvl(ACC.SOURCE_ACCOUNT_AID,ACC.source_gigya_aid) as CUSTOMER_ID
...
,EA.ACCOUNT_ID
,ACC.ACCOUNT_SPECIALTY
,nvl(ACC.source_gigya_aid,ACC.SOURCE_ACCOUNT_AID)
,ACC.SOURCE_GIGYA_AID
...
SELECT 
            nvl(nvl(DCA.SOURCE_ACCOUNT_AID,PH.SOURCE_GIGYA_AID),PH.POST_VISID_HIGH || '_' || PH.POST_VISID_LOW) as CUSTOMER_ID
            ,PH.SOURCE_GIGYA_AID
            ,(PH.POST_VISID_HIGH || '_' || PH.POST_VISID_LOW) AS VISITOR_ID
...
# MAGIC %sql
# MAGIC 
# MAGIC select * from slides_per_hcp
# MAGIC where country_id=27
# MAGIC and calen_mo=8
# MAGIC and calen_year=2020
# MAGIC and global_product_id=3
# MAGIC and document_number='PM-BR-FVU-EDTL-190006'
# MAGIC and customer_id='0016100000NoclAAAR'
# MAGIC --and customer_id = '0016100000NodoPAAR'
